...
@push('styles')
<script src="https://cdn.jsdelivr.net/npm/@tailwindcss/browser@4.3.0/dist/index.global.min.js">
...
@section('content')
<div
    class="bg-white p-6 rounded-xl shadow-xs max-w-2xl mx-auto border border-gray-100"
    x-data="restoreDashboard()"
>

    <h2 class="m-0 mb-1.5 text-2xl font-bold text-gray-900">Restore Database Backup</h2>
    <p class="text-gray-500 text-sm mb-6">Upload a ZIP archive containing <code>table.json</code> and CSV files per
        table.</p>

    <div
        class="mb-4 p-3 px-4 bg-amber-50 border border-amber-300 rounded-lg text-amber-800 text-sm"
        x-show="nginxBanner"
        x-cloak
    >
        Server rejected the request. File may be too large (server limit). Try a smaller backup or increase
        <code>client_max_body_size</code>.
    </div>

    <div
        class="border-2 border-dashed border-indigo-100 rounded-xl p-7 text-center cursor-pointer transition-colors bg-slate-50/50 hover:bg-indigo-50/40 hover:border-indigo-400 group"
    ...
    >
        ...
        <div class="text-sm text-gray-600">
            Drag &amp; drop your ZIP here, or <span
                class="text-indigo-500 font-semibold cursor-pointer"
                @click="$refs.fileInput.click()"
            >browse</span>
        </div>
        <div
            class="mt-2 text-xs text-indigo-500 font-medium min-h-[18px]"
            x-text="fileName ? `📎 ${fileName}` : ''"
        >
        </div>
    </div>

    <button
        class="inline-flex items-center gap-2 px-6 py-2.5 bg-indigo-500 hover:bg-indigo-600 disabled:opacity-55 disabled:cursor-not-allowed text-white border-none rounded-lg text-sm font-semibold cursor-pointer transition-colors mt-5"
    ...
    >
        <div class="bg-indigo-50 rounded-full h-2 overflow-hidden mb-1.5">
            <div
                class="h-full bg-gradient-to-r from-indigo-500 to-indigo-400 rounded-full transition-[width] duration-300 ease-in-out"
                :style="`width: ${progress.percentage}%`"
            ></div>
        </div>
        <div
            class="text-xs text-gray-400 text-right"
            x-text="`${progress.done} / ${progress.total}`"
        >0 / 0</div>
    </div>

    <div
        class="mt-6 rounded-xl border border-gray-200 bg-gray-50 max-h-[500px] overflow-y-auto p-4 text-sm leading-relaxed text-gray-700"
        x-ref="logContainer"
    >
            ...
            <div>
                ...
                <template x-if="log.type === 'group'">
                    <div class="my-1 border border-gray-200 rounded-md overflow-hidden">
                        <div
                            class="flex items-center gap-2 p-2 px-3 bg-gray-100 hover:bg-gray-200 cursor-pointer select-none font-semibold text-sm transition-colors"
                            @click="log.expanded = !log.expanded"
                        >
                            <span
                                class="text-[0.7rem] text-gray-400 transition-transform duration-200"
                                :class="{ '-rotate-90': !log.expanded }"
                            >▼</span>
                            📦 <span x-text="log.tableName"></span>
                            <span
                                class="text-xs font-normal text-gray-500 ml-auto"
                                x-text="`${log.doneCount} / ${log.chunkCount}`"
                            ></span>
                        </div>
                        <div
                            class="bg-white"
                        ...
                        >
                            <template
                                x-for="chunk in log.chunks"
                                :key="chunk.id"
                            >
                                <div
                                    class="flex items-baseline justify-between p-1.5 pr-3 pl-7 border-t border-gray-100 text-xs">
                                    <span class="text-gray-700">📄 <span x-text="chunk.name"></span></span>
                                    <span
                                        class="text-xs font-bold px-2 py-0.5 rounded-full whitespace-nowrap ml-2.5 shrink-0"
                                        :class="chunk.badgeClass"
                                        x-text="chunk.badgeText"
                                    ></span>
                                </div>
                            </template>
                        </div>
            ...
            return {
                ...
                async buildSchemaMap(zip) {
                    ...
                    try {
                        ...
                        this.appendHtmlLog(
                            `<span class="text-red-700">Could not parse table.json: ${err.message}</span>`);
                        return null;
                    }
                },
                ...
                async startRestoreProcess() {
                    ...
                    const csvFiles = Object.values(zip.files).filter(f => !f.dir && f.name.toLowerCase().endsWith(
                        '.csv'));
                    ...
                    this.appendHtmlLog(
                        `Found <strong>${csvFiles.length}</strong> CSV file(s).${schemaMap ? ` Schema loaded for <strong>${Object.keys(schemaMap).length}</strong> table(s).` : ' No <code>table.json</code> found — tables must already exist.'}<br>`
                        );
                    ...
                    for (let gi = 0; gi < groups.length; gi++) {
                        ...
                        for (let ci = 0; ci < group.files.length; ci++) {
                            // Throttle: wait 1 second (1000ms) before every request except the very first one
                            if (ci > 0 || gi > 0) {
                ...
                async uploadAndRestoreFile(file, tableSQL, chunkState) {
                    ...
                    try {
                        ...
                        if (contentType.includes('text/html') || contentType.includes('text/plain')) {
                            ...
                            if (text.includes('nginx') || text.includes('413') || text.includes(
