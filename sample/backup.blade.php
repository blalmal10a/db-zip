...
<div class="card">
    ...
    <div id="existing-backups-container" style="margin-top: 24px; display: none;">
        <h3 style="font-size: 1rem; margin-bottom: 12px; color: #444;">Available Past Backups</h3>
...
<script>
    ...
    // Run on page load to pull the initial list of tables and zip files
    ...
    // Loops through the files string array and builds individual download-rows
    function renderExistingFiles(files) {
        existingList.innerHTML = ''; // Clear prior elements

        files.forEach(filePath => {
            // Extract file name from the path string (e.g., '1780997119820.zip')
            const timestamp = filePath.split('/').pop().replace('.zip', '');
            const dt = new Date(parseInt(timestamp));

            const fileName = `${toHyphenDateTime(dt)}.zip`;

            // Build absolute application URL structure based on public/storage
            const webPath = filePath.replace('public/', '/');

            const row = document.createElement('div');
            row.className = 'download-row visible';
            row.style.marginTop = '8px'; // tight grouping

            row.innerHTML = `
                <div class="download-filename">
                    &#128230; <span>${fileName}</span>
                </div>
                <a class="download-btn" href="${webPath}" download="${fileName}">&#8595; Download</a>
            `;
            // add delete button
    ...
    async function startBackupProcess() {
        ...
        try {
            ...
            // Separate fallback if Laravel returns object or straight table array 
            ...
            // Refresh the existing files array view list if updated concurrently
            if (data.files) {
                renderExistingFiles(data.files);
            }
        } catch (error) {
    ...
    async function zipFolder() {
        try {
            ...
            // Re-trigger initialize to pull down the newly added file into the list view
            initializeBackupDashboard();
            setTimeout(() => {
                fetch('delete-backup-folder');
            }, 1500);
    ...
    function toHyphenDateTime(date = new Date()) {
        ...
        // Return just the date, or append the time depending on your needs
        return `${year}-${month}-${day} ${hours}-${minutes}-${seconds}`;
    }

    async function deleteZipByFilename(fileName) {
        fetch('delete-zip-file-by-name', {
            method: 'DELETE',
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                'Content-Type': 'application/json'
